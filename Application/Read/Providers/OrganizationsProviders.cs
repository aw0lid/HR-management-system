...
namespace Application.Read.Providers
{
   public abstract class LookupProvider<TEntity, TView> where TEntity : class
    ...
    public class PermissionProvider : LookupProvider<Permission, PermissionView>
    {
        public PermissionProvider(CacheProvider<Permission> cache) : base(cache){}

        public async Task<Result<List<PermissionView>>> GetAll() 
            => await GetAsync(new Error("PERMISSIONS_EMPTY", enErrorType.Validation));

        protected override PermissionView Mapping(Permission entity)
        {
